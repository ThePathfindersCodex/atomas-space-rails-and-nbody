...
func redraw():
	...
	if ! parent_body is Star:
		$Sprite.rotation = parent_body.orbitalVelocityf2.angle() + deg2rad(120)	
		
		var tmpStar = parent_body
		while !tmpStar.get_parent() is Star:
			tmpStar = tmpStar.get_parent()
			$Sprite.rotation = tmpStar.orbitalVelocityf2.angle() + deg2rad(120)	
...
#FINAL - SET PRIMARY ELEMENTS	
	...
	#	trueAnomaly
	if v <= PI:
		trueAnomaly = rad2deg(v)
	else:
		trueAnomaly = rad2deg(v-(2*PI))
...
# MORE ORBITAL VALUES
	...
	var orbitalDistance = semiMajorAxis*((1-(pow(eccentricty,2)))/(1+eccentricty*cos(deg2rad(trueAnomaly))))
	var meanAnomalyMotion = (2*PI) / orbitalPeriod
	eccentricAnomaly = rad2deg(2*atan( sqrt( (1-e)/(1+e) ) * tan(deg2rad(trueAnomaly)/2) ) )
	meanAnomaly=rad2deg(deg2rad(eccentricAnomaly)-(eccentricty*sin(deg2rad(eccentricAnomaly))))

	# SOI
	#	SOIsize=  semiMajorAxis* pow(self_mass/parent_body.self_mass,2/5)  
	SOIsize=  orbitalDistance * pow(self_mass/parent_body.self_mass,2/5) * .6
...
# CALC ORBIT ELLIPSE - TODO hyperb and esc vel, etc	
	...
	if vel<escapeVelocityMag:
		...
		# VELOC AND NORMALS
		orbitalVelocityf1 = position.direction_to(ellipse.get_focus1()) *100 * $Sprite.scale.x
		orbitalVelocityf2 = position.direction_to(ellipse.get_focus2()) *100 * $Sprite.scale.x
		orbitalVelocityNormal = (orbitalVelocityf1+orbitalVelocityf2).normalized() *100 * $Sprite.scale.x
